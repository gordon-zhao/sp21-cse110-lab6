...
class JournalEntry extends HTMLElement {
  ...
  set entry(entry) {
    ...
    // CODE GOES HERE
    let entryRoot = this.shadowRoot.querySelector('.entry')
    let children = entryRoot.children;
    for (let index = 0; index < children.length; index++) {
      children[index].textContent = entry[children[index].className.split('-')[1]];
    }
    ...
    if (entry.image) {
      ...
      entryRoot.appendChild(entryImage);
    ...
    if (entry.audio) {
      ...
      entryRoot.appendChild(entryAudio);
